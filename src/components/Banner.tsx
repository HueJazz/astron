...
const Banner = () => {
    ...
    return(
        <div className="custom-px pt-6 pb-2 custom-max-w mx-auto">
            <div className="flex justify-start relative overflow-hidden p-22 rounded-3xl banner" id="banner">
                <video className='absolute min-w-max z-0 top-0 left-1/2 -translate-x-1/2 brightness-50' muted loop autoPlay>
                    <source src={require(".././images/video.mp4")} type="video/mp4; codecs=&quot;avc1.42E01E, mp4a.40.2&quot;" />
                </video>
                <div className="min-w-full flex flex-row justify-between items-end text-white">
                    <div className="z-0 mb-6 flex flex-col gap-6">
                        <h1 className="lg:text-7xl text-3xl font-bold">Guardians of your digital realm</h1>
                        <p className="lg:text-2xl text-lg">Stay a Step Ahead of Cyber Threats with Revolutionary Cybersecurity Solutions from αstron. Safeguarding Your Data, One Click at a Time.</p>
                        <Link to="contact" spy={true} smooth={true} duration={750} key="contact">
                            <button className="duration-200 custom-bg text-white rounded-2xl px-6 py-1 font-medium leading-6 hover:bg-white hover:text-blue-800">
                                Get in touch
                            </button>
                        </Link>
                    </div>
                    <ul className="flex flex-col gap-12 pr-6">
